...
u8 sst25vfDesc( PYGMYSPIPORT *pygmySPI, PYGMYMEMDESC *pygmyDesc )
{
    ...
    pygmySPI->PortCS->BSRR = pygmySPI->PinCS;
    /*print( STDIO, "\rCapacity: %d", pygmyDesc->Capacity );
    print( STDIO, "\rSectors: %d", pygmyDesc->Sectors );
    print( STDIO, "\rSectorSize: %d", pygmyDesc->SectorSize );
    print( STDIO, "\rAttributes: %d", pygmyDesc->Attributes );
    print( STDIO, "\rManufacturer: %d", pygmyDesc->Manufacturer );
    print( STDIO, "\rType: %d", pygmyDesc->Type );
    */
    return( TRUE );
}

    ...
    if( XModem->Status & XMODEM_RECV ){ // RECV      
        if( XModem->Status & XMODEM_MODE_SOH ){ // Recieving Payload, after <SOH>
            ...
            if( Byte == XMODEM_SOH ){ // This is XModem <SOH> // was 0x01
                ...
                //delay(500);
                print( STDIO, "\rClosing File %s", XModem->File->Properties.Name );
                //putstr( "Done\r> " );
            } // else if
            XModem->Index = 0;
        } // else
        ...
        if( Byte == XMODEM_ACK ){
            ...
            xmodemInit( XModem );
            //XModem->Status = 0;
            print( STDIO, "%c", 0x06 ); // Send Ack to close connection
            //putstr( (u8*)BOOT_PROMPT );
        } // else if
        return( TRUE );
    } // else if

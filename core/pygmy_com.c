...
u8 comGetProtocol( void )
{

}

u8 comSetProtocol( u8 )
{

}

u8 comGetDatabits( void )
{

}

u8 comSetDatabits( u8 )
{

}

u8 comGetParity( void )
{
...
u8 comGetStopbits( void )
{

}

u8 comSetStopbits( u8 )
{

}

u8 comGetFlowcontrol( void )
{

}

u8 comSetFlowcontrol( u8 )
{
...
u8 comOpen( u8 Port, u32 BaudRate, u8 Options, u32 MainClock )
{
    USART_TYPEDEF *Uart;
    //u32 MainClock;
...
u8 streamReset( u8 ucStream )
{
    if( ucStream < MAXCOMPORTS ){
        ...
        globalStreams[ ucStream ].Pipe          = NOSTREAM;
...
void streamSetSTDIO( u8 ucStream )
{
    ...
    globalStreams[ STDIO ].Pipe         = globalStreams[ ucStream ].Pipe;
...
{
    if( globalStreams[ ucStream ].Pipe != NOSTREAM ){
        ucStream = globalStreams[ ucStream ].Pipe;
    } // if
    if( ucStream < MAXCOMPORTS ){
        globalStreams[ ucStream ].RXIndex = 0;
        globalStreams[ ucStream ].RXLen = 0;
...
void streamSetPipe( u8 streamFrom, u8 streamTo )
{
    // example: COM1->RX -> COM2->TX
    //          COM1->TX -> COM2->TX
    //          COM2->RX -> COM1->TX
    //          COM2->TX -> COM1->TX
    globalStreams[ streamTo ].Pipe = streamFrom;
    globalStreams[ streamFrom ].Pipe = streamTo;
}
...
{   
    /*if( ucStream < MAXCOMPORTS {
        if( globalStreams[ ucStream ].Pipe != NOSTREAM ){
            if( globalStreams[ globalStreams[ ucStream ].Pipe ].TXLen ){
		--globalStreams[ globalStreams[ ucStream ].Pipe ].TXLen;  
		((USART_TYPEDEF *)pygmyPeriph)->DR = globalStreams[ globalStreams[ ucStream ].Pipe ].TXBuffer[ globalStreams[ ucStream ].TXIndex ]; 
                globalStreams[ ucStream ].TXIndex = ( globalStreams[ globalStreams[ ucStream ].Pipe ].TXIndex + 1 ) % 
                    globalStreams[ globalStreams[ ucStream ].Pipe ].TXBufferLen;
            } else{
                ((USART_TYPEDEF *)pygmyPeriph)->CR1 &= ~USART_TXEIE;
            } // else
        } else{
            if( globalStreams[ ucStream ].TXLen ){
...
__attribute__ (( weak )) void USART1_IRQHandler( void )
{
    ...
    if( USART1->SR & USART_RXNE){
        if( globalStreams[ globalStreams[ COM1 ].Pipe ] ){
            streamTXChar( COM1, globalStreams[ globalStreams[ COM1 ].Pipe ].UART );
        } else if( globalStreams[ COM1 ].CR & PYGMY_STREAMS_USERHANDLER ){

...
void filePrintProperties( PYGMYFILEPROPERTIES *Properties )
{
    ...
    print( STDIO, "\rID: 0x%08X", Properties->ID );
    print( STDIO, "\rTime: %t", Properties->Time );
    print( STDIO, "\rLength: %d", Properties->Length );
    print( STDIO, "\rName: %s", Properties->Name );
    print( STDIO, "\rPath: %s", Properties->Path );
    print( STDIO, "\rAttributes:" );
    ...
    print( STDIO, "\rSectorGroups: %d", Properties->SectorGroups );
    print( STDIO, "\rSectorCount: %d", Properties->SectorCount );
    for( i = 0; i < Properties->SectorGroups; i++ ){
        print( STDIO, "\r\tBaseAddress: 0x%012llX", Properties->Sectors[ i ].BaseAddress );
        print( STDIO, "\r\tSectorCount: %d", Properties->Sectors[ i ].SectorCount );
    } // for
}
...
u8 *fileCreateNameWithPath( u8 *Name )
{
    ...
    //print( STDIO, "\rCreated NameWithPath: %s", NameWithPath );
...
u8 *fileGetBoundedString( PYGMYFILE *File, u8 *PreString, u8 *PostString )
{
    // This function seeks the NEXT occurrence of PreString and copies the string up to the start of PostString
    // PreString and PostString must not be the same
    // Both PreString and PostString must be found before the string is loaded
    // The string returned must be allocated by this function and must be freed by the calling function
    // This function will return NULL if it is unable to meet the above criteria or insufficient memory s available
    u32 BoundedStringLen, PreStringIndex;
    u8 *BoundedString;

    if( PreString == NULL || PostString == NULL || isStringSame( PreString, PostString ) ){
        return( NULL );
    } // if
    
    if( !fileSeekString( File, PreString ) ){
        return( NULL );
    } // if

    PreStringIndex = File->Index;

    if( !fileSeekString( File, PostString ) ){
        return( NULL );
    } // if
 
    BoundedStringLen = ( File->Index - len( PostString ) ) - PreStringIndex;
    // fileGetBuffer requires an allocated block of memory to store the 
    BoundedString = malloc( BoundedStringLen + 1 );
    if( BoundedString == NULL ){
        return( NULL );
    } // if
    if( fileGetBuffer( File, BoundedString, BoundedStringLen ) ){
        BoundedString[ BoundedStringLen ] = '\0';
...
u8 fileSeekString( PYGMYFILE *File, u8 *StringToFind )
{
    // This function positions the file index after the NEXT occurrence of StringToFind
    // If no match is found before reaching EOF, the function will restore the file index and return FALSE
    u32 Index, i, Length;
    u8 TmpChar;
    ...
    for( i = 0; !(File->Properties.Attributes & EOF ) ; ){
        fileReadBytes( File, &TmpChar, 1 );
        if( TmpChar != StringToFind[ i++ ] ){
            i = 0;
        } // if
        if( i == Length ){
            // If i equals Length, then we have a match
            return( TRUE );
        } // if
    } // for
...
u8 fileSeekChar( PYGMYFILE *File, u8 CharToFind )
{
    // This function positions the file index after the NEXT occurrence of CharToFind
    // To search for the first occurrence, you must set the file index to 0 first
    u32 Index;
    u8 TmpChar;

    Index = File->Index;
    for( ; !(File->Properties.Attributes & EOF); ){
        fileReadBytes( File, &TmpChar, 1 );
    ...
    File->Index = Index; // No match was found, restore file index
    return( FALSE );
}
    ...
    for( i = 0; i < MaxFiles; i++ ){
        ...
        //if( !Properties.Attributes & FOLDER ){
            if( !strcmp( FileName, Properties.Name ) ){
                print( STDIO, "\rFound %s @ %s", FileName, Volume->Properties.Path );
